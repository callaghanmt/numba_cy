...
Whereas Cython can be used to create a build a compiled C version of a module (or any chunk of Python code) as a seperate build event, **numba** uses a process called **Just In Time** compilation (or **jit**) to compile the required part of a Python program as and when it is needed.
...
Once numba is installed, it takes just three additional lines to interface the cose with the LLVM compiler:
